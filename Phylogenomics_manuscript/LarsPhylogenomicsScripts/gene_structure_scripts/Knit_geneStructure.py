"""-----------------------------------------------------------------------------
  * Script Name: Knit_geneStructure.py
...
file = open("Knit_BAHDs.txt", 'r') 
gList = [line.rstrip("\n") for line in file]

newFile = open("Knit_results.txt", "w")
...
for line in open(sys.argv[1]):
	if line.startswith("#"):
		...
		continue
	test = re.split('[\t=;]', line) 
	if test[9] in gList and test[2] == "mRNA":
	...
	if test[2] == "mRNA" and test[9] not in gList:
		if found:
			newFile.write(ID + '\t' + str(fLen) + '\t' + str(cLen) + '\t' + str(iLen) + '\t' + str(iNum) + '\t' + str(iLista) + '\t' + str(iListb) + '\n')
			found = False
			count = fLen = cLen = iLen = iNum = 0; iLista = []; iListb = [];	
	if found:	
		if test[2] == "exon":
		...
		count = fLen = cLen = iLen = iNum = 0; iLista = []; iListb = []; ID = ""; curGene = False
		continue

...
file = open('{0}_BAHDs.txt'.format(sys.argv[2][:4]), 'r') 
gList = [line.rstrip("\n") for line in file]

newFile = open('{0}_results.txt'.format(sys.argv[2][:4]), "w")
		...
		if curGene:
			...
			if found:
				...
				if test[9] in gList:
					...
					#print("BAHD was found at:" + str(test))
